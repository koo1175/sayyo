...
export default function Politician() {
    ...
    return (
        <div style={{ display: 'flex', marginTop: 50, width: 1600, marginLeft: '-10%', flexDirection: 'column' }}>
            <div style={{ display: 'flex' }}>
                <img src='img/backgroundimg.png' style={{ position: 'absolute', top: 400, left: 400, zIndex: '-1' }}></img>
                <div style={{ height: 600, width: 750, position: 'relative', borderBottomRightRadius: '30%', overflow: 'hidden' }}>
                    <img src={`/img/${text}시장2.jpg`} style={{ width: '100%', height: '100%', objectFit: 'cover' }}></img>
                    <div style={{ position: 'absolute', bottom: 0, height: '30%', width: '100%', backgroundImage: 'linear-gradient(rgba(0,0,0,0), rgba(0,0,0,1))' }} />
                </div>
                <div style={{ height: 600, width: 800, flexDirection: 'column' }}>
                    <div style={{ display: 'flex', justifyContent: 'start', marginTop: 20, marginLeft: 50 }}>
                        <img src="/img/유튜브로고.png" alt="설명" className="logo" onClick={() => openLink(data?.youtube)}/>
